...
for base in candidate_dirs:
    APP_PATHS.append(base / 'AppMobileFixed.js')
    APP_PATHS.append(base / 'App.js')
...
if not existing_paths:
    raise SystemExit('mobile AppMobileFixed.js/App.js not found')
...
function TeachersManagementScreen({ token, onBack }) {
  ...
  async function saveTeacher() {
    ...
    try {
      ...
      await requestJson(editing ? `/teachers/${editing.id}` : '/teachers', {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenPad} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <AdminBackHeader title="إدارة المعلمات" onBack={onBack} />
      ...
      <View style={styles.managerFormCard}>
        <View style={styles.managerFormHeader}>
          <TouchableOpacity style={[styles.iconAction, styles.saveIconAction]} onPress={saveTeacher} disabled={saving} activeOpacity={0.8}>
            {saving ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="save-outline" size={22} color="#fff" />}
          </TouchableOpacity>
          {editing ? (
            <TouchableOpacity style={[styles.iconAction, styles.cancelIconAction]} onPress={resetForm} activeOpacity={0.8}>
              <Ionicons name="close-outline" size={24} color={C.red} />
            </TouchableOpacity>
          ) : null}
          <View style={styles.managerFormTitleWrap}>
            <Text style={styles.managerFormTitle}>{editing ? 'تعديل معلمة' : 'إضافة معلمة جديدة'}</Text>
          ...
          {teachers.length === 0 ? <EmptyRow title="لا توجد معلمات" /> : teachers.map((teacher, idx) => (
            <View key={teacher.id} style={[styles.managerRow, idx === teachers.length - 1 && { borderBottomWidth: 0 }]}>
              <View style={styles.managerRowActions}>
                <TouchableOpacity style={styles.rowIconBtn} onPress={() => startEdit(teacher)} activeOpacity={0.8}>
...
function CriteriaManagementScreen({ token, onBack }) {
  ...
  async function saveCriterion() {
    ...
    setSaving(true);
    try {
      await requestJson(editing ? `/evidence/${editing.id}` : '/evidence', {
        method: editing ? 'PUT' : 'POST',
        token,
        body: { title: title.trim(), description: description.trim() },
      });
      Alert.alert('تم', editing ? 'تم تعديل المعيار' : 'تمت إضافة المعيار');
      resetForm();
      await load();
    } catch (error) {
      Alert.alert('تعذر الحفظ', error.message);
  ...
  function confirmDelete(item) {
    Alert.alert('حذف المعيار', `هل تريد حذف ${item.title}؟ سيتم حذف الملفات المرتبطة به.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        text: 'حذف',
        style: 'destructive',
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenPad} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <AdminBackHeader title="إدارة المعايير" onBack={onBack} />
      <Text style={styles.pageSubtitle}>إنشاء وتعديل وحذف المعايير</Text>

      <View style={styles.managerFormCard}>
        <View style={styles.managerFormHeader}>
          <TouchableOpacity style={[styles.iconAction, styles.saveIconAction]} onPress={saveCriterion} disabled={saving} activeOpacity={0.8}>
            {saving ? <ActivityIndicator color="#fff" size="small" /> : <Ionicons name="save-outline" size={22} color="#fff" />}
          </TouchableOpacity>
          {editing ? (
            <TouchableOpacity style={[styles.iconAction, styles.cancelIconAction]} onPress={resetForm} activeOpacity={0.8}>
          ...
          <View style={styles.managerFormTitleWrap}>
            <Text style={styles.managerFormTitle}>{editing ? 'تعديل معيار' : 'إضافة معيار جديد'}</Text>
            <Text style={styles.managerFormSub}>يمكن ترك الوصف فارغًا</Text>
          </View>
        </View>

        <TextInput style={styles.managerInput} value={title} onChangeText={setTitle} placeholder="اسم المعيار" placeholderTextColor={C.subtle} textAlign="right" />
        <TextInput style={[styles.managerInput, styles.managerTextarea]} value={description} onChangeText={setDescription} placeholder="وصف المعيار" placeholderTextColor={C.subtle} multiline textAlign="right" textAlignVertical="top" />
      </View>

      <Text style={styles.sectionLabel}>قائمة المعايير</Text>
      {loading ? (
        <ActivityIndicator color={C.primary} size="large" style={{ marginTop: 30 }} />
      ) : (
        <View style={styles.listCard}>
          {items.length === 0 ? <EmptyRow title="لا توجد معايير" /> : items.map((item, idx) => (
            <View key={item.id} style={[styles.managerRow, idx === items.length - 1 && { borderBottomWidth: 0 }]}>
              <View style={styles.managerRowActions}>
                <TouchableOpacity style={styles.rowIconBtn} onPress={() => startEdit(item)} activeOpacity={0.8}>
                  <Ionicons name="pencil-outline" size={20} color={C.primary} />
                </TouchableOpacity>
                <TouchableOpacity style={styles.rowIconBtn} onPress={() => confirmDelete(item)} activeOpacity={0.8}>
                  <Ionicons name="trash-outline" size={20} color={C.red} />
                </TouchableOpacity>
              </View>
              <View style={styles.managerRowText}>
                <Text style={styles.managerRowTitle}>{item.title}</Text>
                <Text style={styles.managerRowSub}>الملفات: {item.uploads_count ?? 0}</Text>
              </View>
              <View style={[styles.actionRowIcon, { backgroundColor: C.tealLight }]}>
                <Ionicons name="list-outline" size={20} color={C.teal} />
              </View>
            </View>
          ))}
        </View>
    ...
    <ScrollView contentContainerStyle={styles.screenPad} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <LinearGradient colors={C.grad} style={styles.profileCard}>
        ...
        <View style={styles.profileSchoolRow}>
          ...
          <Text style={styles.profileSchoolName}>{user?.school?.name || 'مدرسة المراسلات'}</Text>
        </View>
      </LinearGradient>
      ...
      {isPrincipal ? (
        <>
          ...
          <View style={styles.listCard}>
            <ActionRow icon="people-outline" title="إدارة المعلمات" subtitle="إضافة وتعديل وحذف المعلمات" accent={C.primary} onPress={onOpenTeachers} />
            <ActionRow icon="list-outline" title="إدارة المعايير" subtitle="إنشاء وتعديل وحذف المعايير" accent={C.teal} onPress={onOpenCriteria} noBorder />
          </View>
        </>
      ) : (
        <>
...
EXTRA_STYLES = r'''
  ...
  managerTextarea: { minHeight: 92, paddingTop: 14 },
...
def replace_required(pattern, repl, text, label):
    new_text, count = re.subn(pattern, repl, text, count=1, flags=re.S)
    if count != 1:
        raise SystemExit(f'Patch failed in {label}: pattern not found')
    return new_text
...
for p in existing_paths:
    ...
    text = text.replace("user?.school?.name || 'مدرسة'", "user?.school?.name || 'مدرسة المراسلات'")

    if 'function TeachersManagementScreen({ token, onBack })' in text and 'function TeacherFilesScreen' in text:
        text = replace_required(
            r"function TeachersManagementScreen\(\{ token, onBack \}\) \{.*?\n\}\n\nfunction TeacherFilesScreen",
            TEACHERS_SCREEN.strip() + "\n\nfunction TeacherFilesScreen",
            text,
            str(p),
        )

    if 'function CriteriaManagementScreen' not in text:
        text = text.replace('\nfunction SettingsScreen', '\n' + CRITERIA_SCREEN.strip() + '\n\nfunction SettingsScreen', 1)

    if 'function SettingsScreen' in text and 'function BottomNav' in text:
        text = replace_required(
            r"function SettingsScreen\(\{.*?\}\) \{.*?\n\}\n\nfunction BottomNav",
            SETTINGS_SCREEN.strip() + "\n\nfunction BottomNav",
            text,
            str(p),
        )

    if "settingsSub === 'criteria'" not in text:
        teachers_route = "else if (tab === 'settings' && settingsSub === 'teachers') screen = <TeachersManagementScreen token={token} onBack={() => setSettingsSub(null)} />;"
        criteria_route = teachers_route + "\n  else if (tab === 'settings' && settingsSub === 'criteria') screen = <CriteriaManagementScreen token={token} onBack={() => setSettingsSub(null)} />;"
        if teachers_route in text:
            text = text.replace(teachers_route, criteria_route, 1)
        else:
            text = re.sub(
                r"(else if \(tab === 'settings' && settingsSub === 'teachers'\).*?;\n)",
                r"\1  else if (tab === 'settings' && settingsSub === 'criteria') screen = <CriteriaManagementScreen token={token} onBack={() => setSettingsSub(null)} />;\n",
                text,
                count=1,
            )

    text = re.sub(
        r"else if \(tab === 'settings'\) screen = <SettingsScreen user=\{user\} onLogout=\{onLogout\}.*?/>;",
        "else if (tab === 'settings') screen = <SettingsScreen user={user} onLogout={onLogout} onOpenTeachers={() => setSettingsSub('teachers')} onOpenCriteria={() => setSettingsSub('criteria')} />;",
        text,
        count=1,
    )
    ...
    if 'managerFormCard:' not in text:
        idx = text.rfind('\n});')
        if idx == -1:
            raise SystemExit(f'Patch failed in {p}: StyleSheet ending not found')
...
print('Patched Amal mobile management cards in: ' + ', '.join(patched))
