...
text = p.read_text()
p.with_suffix('.js.backup-before-managers-fix').write_text(text)
...
# Insert missing component definitions before TeacherFilesScreen, or before SettingsScreen as fallback.
insert_marker = 'function TeacherFilesScreen'
if 'function TeachersManagementScreen' not in text:
    if insert_marker in text:
        text = text.replace(insert_marker, teacher_screen + '\n\n' + insert_marker, 1)
    else:
        text = text.replace('function MainApp', teacher_screen + '\n\nfunction MainApp', 1)
if 'function CriteriaManagementScreen' not in text:
    if insert_marker in text:
        text = text.replace(insert_marker, criteria_screen + '\n\n' + insert_marker, 1)
    else:
        text = text.replace('function MainApp', criteria_screen + '\n\nfunction MainApp', 1)

# Ensure settingsSub state exists.
...
# Ensure settings screen has the two cards.
settings_screen = r'''
...
'''
if 'title="إضافة معلمة"' not in text or 'title="إدارة المعايير"' not in text:
...
if criteria_route not in text:
    text = text.replace(settings_call, criteria_route + "\n  " + settings_call, 1)

# Ensure goTab closes sub screens.
...
p.write_text(text)
required = ['function TeachersManagementScreen', 'function CriteriaManagementScreen', 'إضافة معلمة', 'إدارة المعايير', 'trash-outline', 'pencil-outline', 'save-outline', "settingsSub === 'teachers'", "settingsSub === 'criteria'"]
missing = [x for x in required if x not in text]
if missing:
    raise SystemExit('Patch failed, missing: ' + ', '.join(missing))
print('mobile managers patch applied to', p)
